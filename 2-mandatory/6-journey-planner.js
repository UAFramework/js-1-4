...
function getTransportModes(londonLocations) {
  return londonLocations.slice(1, londonLocations.length);
  }
...
function isAccessibleByTransportMode(arr, transportMode) {
  return arr.some(item => item === transportMode);
}
...
function journeyPlanner(londonLocations, transportMode) {
  let  famousLocations = [];
  for(let location of londonLocations) {
      let transports = getTransportModes(location);
      let isAccessible = isAccessibleByTransportMode(transports, transportMode);
      
      if (isAccessible) {
        famousLocations.push(getLocationName(location));
      }  
  ...
  // return locations.filter(location => location.join(',').indexOf(transportMode) > 0).map(el => el[0]);
